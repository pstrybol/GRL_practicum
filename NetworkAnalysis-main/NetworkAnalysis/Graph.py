...
class Graph:
    ...
    def __init__(self, interaction_df, colnames=None, verbose=True, keeplargestcomponent=False,
        ...
        self.nodes = np.array([gene2int[s] for s in node_names]).astype(int)
...
def positives_split(interaction_df, all_pos_pairs=None, train_ratio=0.7, train_validation_ratio=None):
    ...
    still_needed_samples = np.maximum(0, np.round(train_ratio * N_edges - len(pos_samples_train)).astype(int))
    ...
    if train_validation_ratio is not None:
        ...
        still_needed_samples_valid = np.maximum(0, np.round(
            train_validation_ratio * N_edges_valid - len(pos_samples_train_valid)).astype(int))
