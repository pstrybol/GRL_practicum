from NetworkAnalysis.InteractionNetwork import positives_split, Graph, UndirectedInteractionNetwork,\
    DirectedInteractionNetwork
from NetworkAnalysis.DLembedder import checkInputDict
import pandas as pd
import numpy as np
import warnings
...
class MultiGraph(Graph):
    ...
    def construct_MLT_train_test(self, train_fraction=0.7, mode='MLT', validation_fraction=None, neg_pos_ratio=5.,
                                    fill_with_NaNs=False, exclude_sets=None, allow_self_connected=True, random_state=23):
        """
        ...
        :param mg: the input data, a MultiGraph object.
        :param test_fraction: fraction of data used for testing. can be a float or a dict of floats. The percentage provided
        will be the percentage of X_train. X_test will consist of 1-test_fraction.
        :param mode: whether to return the sets in MLT mode or not.
        If mode is MLT (multiloss training) then the labels are returned as a dictionary
        and this data can immediately be used in keras for multiloss training.
        if not, the labels are returned as a pandas DF,
        so that the user can decide how to aggregate into a single label for single loss training.
        :param validation_fraction: fraction of data used for validation, this fraction is taken after the train_test split.
        can be a float or a dict of floats. The percentage provided will be the percentage of X_val. X_train will
        consist of 1-validation_fraction.
        :param directed: whether the input DFs should be interpreted as directed or undirected Datasets.
        Only used when the input is a pandas edgelist.
        :param neg_pos_ratio: negative to positive ratio. can be a numeric or a dict of numerics.
        :param fill_with_NaNs: whether the output labels should contain NaNs or zeros when unknown.
        :return: the train, test and validation data. If the user
        """
        # TODO: check interpretation of validation_fraction in subcode
        # TODO: implement for DirectedNetworks

        np.random.seed(random_state)

        if (mode.upper() != 'MLT') and fill_with_NaNs:
            warnings.warn('The <fill_with_NaNs> argument is only meant for MLT training, '
                          'be aware that the returned labels will contain NaNs.')

        model_names = list(self.int2edgetype.values())  # TODO change to graph name

        train_fraction = checkInputDict(train_fraction, float, model_names, 'train_fraction', allowed_type_name='float')
        # directed = checkInputDict(directed, bool, model_names, 'directed', allowed_type_name='bool')
        neg_pos_ratio = checkInputDict(neg_pos_ratio, float, model_names, 'neg_pos_ratio', allowed_type_name='float')

        if (validation_fraction is not None) and (validation_fraction > 1e-5):
            validation_fraction = checkInputDict(validation_fraction, float, model_names,
                                                 'validation_fraction',
                                                 allowed_type_name='float')

        else:
            validation_fraction = {modelname_: None for modelname_ in model_names}

        X_train, X_val, X_test, Y_train, Y_val, Y_test = {}, {}, {}, {}, {}, {}

        # STEP 1: calculate MSTree
        mstree_edgelist = self.getMinimmumSpanningTree(as_edge_list=True, return_names=True)

        for k in model_names:
            obj = self.getEdgeType_subset(k, allow_self_connected=allow_self_connected)
            func_ = lambda x: obj.int2gene[x]
            vf = np.vectorize(func_) # is this still necessary? can't we use the int2gene from multigraph

            all_pos, all_neg = obj.sample_positives_and_negatives(neg_pos_ratio=neg_pos_ratio[k],
                                                                  return_names=False)

            if validation_fraction[k] is None:

                X_train_, X_test_,\
                Y_train[k], Y_test[k] = obj.getTrainTest(mstree=mstree_edgelist,
                                                         all_pos=all_pos, all_neg=all_neg,
                                                         train_ratio=train_fraction[k],
                                                         train_validation_ratio=validation_fraction[k],
                                                         excluded_sets=exclude_sets[k])

                X_train[k], X_test[k] = vf(X_train_), vf(X_test_)

                if exclude_sets[k] is not None:
                    assert not set(map(tuple, X_train[k])) & set(exclude_sets[k]), f"{k}: Overlap train - exclude sets"
                    assert not set(map(tuple, X_test[k])) & set(exclude_sets[k]), f"{k}: Overlap test - exclude sets"

            else:
                X_train_, X_val_, X_test_, Y_train[k], Y_val[k], \
                Y_test[k] = obj.getTrainTest(mstree=mstree_edgelist,
                                             all_pos=all_pos, all_neg=all_neg,
                                             train_ratio=train_fraction[k],
                                             train_validation_ratio=validation_fraction[k],
                                             excluded_sets=exclude_sets[k])
                ...
                if X_val_.shape[0] > 0:
                    X_train[k], X_val[k], X_test[k] = vf(X_train_), vf(X_val_), vf(X_test_)
        ...
        X_train, Y_train = merge_labels_MLT(X_train, Y_train, fill_with_NaNs=fill_with_NaNs)
        X_test, Y_test = merge_labels_MLT(X_test, Y_test, fill_with_NaNs=fill_with_NaNs)
        X_val, Y_val = merge_labels_MLT(X_val, Y_val, fill_with_NaNs=fill_with_NaNs)

        unique_nodes = np.unique(X_train)
        gene2int = {n: i for i, n in enumerate(unique_nodes)}
        func_ = lambda x: gene2int[x]
        vf = np.vectorize(func_)

        X_train, X_test = vf(X_train), vf(X_test)

        if X_val is not None:
            X_val = vf(X_val)

        if mode.upper() != 'MLT':  # This allows the user to choose how to aggregate different labels
        ...
        return X_train, Y_train, X_test, Y_test, X_val, Y_val, gene2int
    ...
    def getTrainTestData(self, train_ratio=0.7, neg_pos_ratio=5., train_validation_ratio=None, excluded_sets=None,
                         return_summary=False, random_state=42, mode="MLT", debug_mode=True):
        as_dicts = True
        if mode.upper() == "SLT":
            as_dicts = False
...
def get_random_rows(arr, fraction1, fraction2=None, as_list_of_tuples=False):
    ...
    N1 = np.int(arr.shape[0] * fraction1)

    if fraction2 is not None:
        N2 = np.int(arr.shape[0] * fraction2)
...
def calculate_corrected_fraction(n_samples, n_MST, orig_fraction):
    ...
    n_test = np.int(np.ceil(orig_fraction * n_samples))
...
def merge_labels_MLT(X_dict, Y_dict, fill_with_NaNs=False):
    ...
    if X_dict is None and Y_dict is None:
        ...
        if fill_with_NaNs:
            ...
            return total_df[['Gene_A', 'Gene_B']].values, \
                   {k: total_df['Y_' + k].values.astype(np.int) for k in X_dict.keys()}
