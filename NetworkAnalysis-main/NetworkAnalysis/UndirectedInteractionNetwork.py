...
from NetworkAnalysis.Graph import Graph, from_df
from NetworkAnalysis.Graph import positives_split, calculateZscores, getDriverScore, FITTERMAPPER, \
...
class UndirectedInteractionNetwork(Graph):
    ...
    def subsetNetwork(self, nodes, inplace=False, verbose=True):
        if inplace:
            ...
            return UndirectedInteractionNetwork(super().subsetNetwork(nodes), node_types=self.node_type_names,
    ...
    def keepLargestComponent(self, verbose=True, inplace=False):
        ...
        if not self.isConnected:
            ...
            if inplace:
                self.__init__(largestcomp.getInteractionNamed(), node_types=self.node_types, gene2int=self.gene2int)
            else:
